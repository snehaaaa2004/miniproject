...
if (!empty($from_time) && !empty($to_time)) {
    // Map ranges to standard slot names
...
// Generate custom therapist ID like THR001, THR002...
...
if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
    ...
    $fullUploadPath = '../' . $uploadDir; // for saving on server
    ...
    if (move_uploaded_file($_FILES['image']['tmp_name'], $targetPath)) {
        $imagePath = $uploadDir . $imageName; // ✅ Store this in DB
    }
}
...
mysqli_stmt_bind_param($stmt, "ssssssssds", $therapistId, $user_id, $gender, $specialization, $experience, $language, $availability, $modeString,$fees, $imageName);
