<?php
// filepath: c:\xampp\htdocs\serenity\admin\admin_messages.php
session_start();
include('../connect.php');
...
if (isset($_POST['delete_id'])) {
    ...
    $stmt_del = $conn->prepare("DELETE FROM therapist_messages WHERE id = ?");
...
$stmt = $conn->prepare("
    SELECT tm.id, tm.therapist_id, tm.user_id, tm.message, tm.admin_reply, tm.created_at, tm.replied_at, u.name AS therapist_name, u.email AS therapist_email
...
<head>
<meta charset="UTF-8">
<title>Therapist Messages - Admin</title>
...
<div class="container">
    <div class="header">
        <h1><i class="fas fa-comments"></i> Therapist Messages</h1>
        <p>Manage and respond to therapist inquiries and support requests</p>
    </div>
            ...
            <div class="message-card" data-id="<?= htmlspecialchars($msg['id']) ?>">
                <div class="message-meta">
                    <div class="therapist-info">
                        <div><strong>Therapist:</strong> <?= htmlspecialchars($msg['therapist_name']) ?></div>
                        <div><strong>Email:</strong> <?= htmlspecialchars($msg['therapist_email']) ?></div>
                        <div><strong>Therapist ID:</strong> <?= htmlspecialchars($msg['therapist_id']) ?></div>
    ...
    <?php else: ?>
        <div class="no-messages">
            ...
            <p>Therapist messages will appear here when therapists contact support.</p>
        </div>
    <?php endif; ?>
</div>
...
<div id="replyModal" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            <h3><i class="fas fa-reply"></i> Reply to Therapist</h3>
        ...
        <form id="replyForm" onsubmit="submitReply(event)">
            ...
            <div id="therapistMessagePreview" style="background: #f8f9fa; padding: 1rem; border-radius: var(--radius); margin-bottom: 1rem; font-style: italic; color: #666;">
                <!-- Therapist message will be displayed here -->
            </div>
            
            <textarea name="reply_message" id="replyText" placeholder="Type your reply to the therapist here..."></textarea>
...
function openReplyModal(messageId) {
    ...
    // Find the message card and get the therapist's message
    const messageCard = document.querySelector(`.message-card[data-id="${messageId}"]`);
    const therapistMessage = messageCard.querySelector('.message-text').textContent;
    const therapistName = messageCard.querySelector('.therapist-info div:first-child').textContent.replace('Therapist:', '').trim();
    
    document.getElementById('therapistMessagePreview').textContent = `Therapist ${therapistName}: "${therapistMessage}"`;
...
function viewTherapistProfile(therapistId) {
    // Redirect to therapist profile page (you can modify this URL as needed)
    window.open(`../admin/view_therapist.php?id=${therapistId}`, '_blank');
}
